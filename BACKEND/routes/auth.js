...
router.post("/login", async (req, res) => {
  ...
  try {
    const [clientRows] = await db.query("SELECT * FROM Client WHERE email = ?", [email]);
    if (clientRows.length === 0) return res.status(400).json({ message: "Invalid credentials" });
    ...
    const token = jwt.sign(
      { client_id: client.client_id, email: client.email, role: client.role },
